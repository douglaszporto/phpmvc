...
require_once dirname(__FILE__) . "/models/produtosCaracteristicas.php";
require_once dirname(__FILE__) . "/models/produtos.php";

use \PHPMVC\DB as DB;

class ProdutoCaracteristicas extends Module{

	public function __construct($key = null){
		$this->model = new ProdutosCaracteristicasModel();
	...
	*/
	public function Grid(&$grid){
		...
		$modelProduto->nProdutoID = $grid->parentIDs[0];

		$titulo = isset($modelProduto->strNome) && $modelProduto->strNome != '' ? (' - '.$modelProduto->strNome) : '';
		$where  = isset($grid->parentIDs[0]) && !empty($grid->parentIDs[0]) ? "WHERE nProdutoID = '".DB::Clean($grid->parentIDs[0])."'" : "";

		$grid->SetTitle('Características do Produto'.$titulo);
		...
		$grid->SetQuery('
			SELECT
				nCaracteristicaID AS "key",
				nCaracteristicaID AS "nCaracteristicaID",
				strTitulo         AS "strTitulo",
				strIdentificador  AS "strIdentificador"
			FROM
				Site.dbo.ProdutosCaracteristicas
			'.$where
		);

		$grid->AddColumnToLayout('nCaracteristicaID','Código','10%');
		$grid->AddColumnToLayout('strTitulo','Produto','50%');
		$grid->AddColumnToLayout('strIdentificador','Identificador','35%');
	}
	...
	*/
	public function Form(&$form, $isUpdate){
		...
		$form->AddField(array(
			...
			'type'     => FORM_TYPE_EDITOR,
			'style'    => 'height:400px',
			'value'    => '<h2>Não se esqueça do título</h2>'
		));
		$form->AddField(array(
		...
		$form->AddField(array(
			...
			'options' => DB::GetValues("Comercial.dbo.Produto","nProdutoSoftwareID","strNome")
		));
		$form->AddField(array(
			'name'    => 'bAtivo',
			'label'   => 'Ativo',
